...
// logger->info("Some buffer {}", lg::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", lg::to_hex(std::begin(buf), std::end(buf)));
// logger->info("Some buffer {:X}", lg::to_hex(std::begin(buf), std::end(buf), 16));
...
template<typename T>
struct formatter<lg::details::dump_info<T>>
{
    ...
    auto format(const lg::details::dump_info<Container> &the_range, FormatContext &ctx) -> decltype(ctx.out())
    {
