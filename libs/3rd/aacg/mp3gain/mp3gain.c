...
void passError( MMRESULT lerrnum, int numStrings, ... ) {
	...
	for ( i = 0; i < numStrings; i++ ) { totalStrLen += strlen( va_arg( marker, const char * ) ); }
	va_end( marker );
	...
	va_start( marker, numStrings );
	for ( i = 0; i < numStrings; i++ ) { strcat( errstr, va_arg(marker, const char *) ); }
	va_end( marker );
...
void crcWriteHeader( int headerlength, char *header ) {
	...
	for ( i = 6; i < headerlength; i++ ) { crc = crcUpdate( ( (unsigned char *)header )[i], crc ); }
...
int deleteFile( char *filename ) { return remove( filename ); }

int moveFile( char *currentfilename, char *newfilename ) { return rename( currentfilename, newfilename ); }
...
int changeGain( char *filename AACGAIN_ARG( AACGainHandle aacH ), int leftgainchange, int rightgainchange ) {
	...
	if ( UsingTemp ) {
		...
		if ( ( filename[outlength - 3] == 'T' || filename[outlength - 3] == 't' ) &&
			...
			( filename[outlength - 1] == 'P' || filename[outlength - 1] == 'p' ) ) { strcat( outfilename, ".TMP" ); }
		else {
				...
				if ( ok ) {
					...
					if ( mpegver == 3 ) {
						...
						if ( !crcflag ) {
							...
							/* WRITETOFILE */
							if ( !UsingTemp ) addWriteBuff( filepos - ( inbuffer - ( curframe + 4 - buffer ) ), curframe + 4 );
						}
					}
					else {
						/* mpegver != 3 */
		...
		if ( !changeGain( filename AACGAIN_ARG( aacH ), leftgainchange, rightgainchange ) ) {
			...
			*/
			...
			if ( leftgainchange == rightgainchange ) {
				...
				if ( tag->haveTrackPeak ) { tag->trackPeak *= pow( 2.0, (double)( leftgainchange ) / 4.0 ); }
				if ( tag->haveAlbumGain ) { tag->albumGain -= dblGainChange; }
				if ( tag->haveAlbumPeak ) { tag->albumPeak *= pow( 2.0, (double)( leftgainchange ) / 4.0 ); }
...
static
void showVersion( char *progname ) { fprintf( stderr, "%s version %s\n", progname,MP3GAIN_VERSION ); }
...
{
	fprintf( stderr, "Usage: appName [options] <infile> [<infile 2> ...]\n" );
	fprintf( stderr, "options:\n" );
	fprintf( stderr, PFX "v - show version number\n", SWITCH_CHAR );
	fprintf( stderr, PFX "g <i>  - apply gain i without doing any analysis\n",SWITCH_CHAR );
	fprintf( stderr, PFX "l 0 <i> - apply gain i to channel 0 (left channel)\n",SWITCH_CHAR );
	fprintf( stderr, "\t          without doing any analysis (ONLY works for STEREO files,\n" );
	fprintf( stderr, "\t          not Joint Stereo)\n" );
	fprintf( stderr, PFX "l 1 <i> - apply gain i to channel 1 (right channel)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "e - skip Album analysis, even if multiple files listed\n",SWITCH_CHAR );
	fprintf( stderr, PFX "r - apply Track gain automatically (all files set to equal loudness)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "k - automatically lower Track/Album gain to not clip audio\n",SWITCH_CHAR );
	fprintf( stderr, PFX "a - apply Album gain automatically (files are all from the same\n",SWITCH_CHAR );
	...
	fprintf( stderr, PFX "i <i> - use Track index i (defaults to 0, for first audio track in file)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "m <i> - modify suggested MP3 gain by integer i\n",SWITCH_CHAR );
	fprintf( stderr, PFX "d <n> - modify suggested dB gain by floating-point n\n",SWITCH_CHAR );
	fprintf( stderr, PFX "c - ignore clipping warning when applying gain\n",SWITCH_CHAR );
	fprintf( stderr, PFX "o - output is a database-friendly tab-delimited list\n",SWITCH_CHAR );
	fprintf( stderr, PFX "t - writes modified data to temp file, then deletes original\n",SWITCH_CHAR );
	fprintf( stderr, "\t     instead of modifying bytes in original file\n" );
#ifdef AACGAIN
	fprintf( stderr, "\t     A temp file is always used for AAC files.\n" );
#endif
	fprintf( stderr, PFX "q - Quiet mode: no status messages\n",SWITCH_CHAR );
	fprintf( stderr, PFX "p - Preserve original file timestamp\n",SWITCH_CHAR );
	fprintf( stderr, PFX "x - Only find max. amplitude of file\n",SWITCH_CHAR );
	fprintf( stderr, PFX "f - Assume input file is an MPEG 2 Layer III file\n",SWITCH_CHAR );
...
#endif
	fprintf( stderr, PFX "? or %ch - show this message\n",SWITCH_CHAR,SWITCH_CHAR );
	fprintf( stderr, PFX "s c - only check stored tag info (no other processing)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "s d - delete stored tag info (no other processing)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "s s - skip (ignore) stored tag info (do not read or write tags)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "s r - force re-calculation (do not read tag info)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "s i - use ID3v2 tag for MP3 gain info\n",SWITCH_CHAR );
	fprintf( stderr, PFX "s a - use APE tag for MP3 gain info (default)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "u - undo changes made (based on stored tag info)\n",SWITCH_CHAR );
	fprintf( stderr, PFX "w - \"wrap\" gain change if gain+change > 255 or gain+change < 0\n",SWITCH_CHAR );
...
int mp3gain_main( int argc, char **argv ) {
	...
	for ( i = 1; i < argc; i++ ) {
		if ( ( ( argv[i][0] == '/' ) || ( argv[i][0] == '-' ) ) &&
			...
			switch ( argv[i][1] ) {
				...
				case 'd':
				case 'D':
					if ( argv[i][2] != '\0' ) { dBGainMod = atof( argv[i] + 2 ); }
					else {
						if ( i + 1 < argc ) {
				...
				case 'G':
					...
					if ( argv[i][2] != '\0' ) { directGainVal = atoi( argv[i] + 2 ); }
					else {
						if ( i + 1 < argc ) {
				...
				case 'H':
				case '?':
					if ( ( argv[i][2] == 'w' ) || ( argv[i][2] == 'W' ) ) wrapExplanation();
					else {
						if ( i + 1 < argc ) { if ( ( argv[i + 1][0] == 'w' ) || ( argv[i + 1][0] == 'W' ) )wrapExplanation(); }
						else fullUsage( argv[0] );
					}
					fullUsage( argv[0] );
					break;
				...
				case 'i':
				case 'I':
					if ( argv[i][2] != '\0' ) { trackIndex = atoi( argv[i] + 2 ); }
					else {
						if ( i + 1 < argc ) {
				...
				case 'm':
				case 'M':
					if ( argv[i][2] != '\0' ) { mp3GainMod = atoi( argv[i] + 2 ); }
					else {
						if ( i + 1 < argc ) {
	...
	if ( databaseFormat ) {
		if ( checkTagOnly ) { fprintf( stdout, "File\tMP3 gain\tdB gain\tMax Amplitude\tMax global_gain\tMin global_gain\tAlbum gain\tAlbum dB gain\tAlbum Max Amplitude\tAlbum Max global_gain\tAlbum Min global_gain\n" ); }
	...
	if ( ( !skipTag ) && ( !deleteTag ) && ( !forceRecalculateTag ) ) {
		...
		for ( mainloop = fileStart; mainloop < argc; mainloop++ ) {
			if ( !maxAmpOnly ) {
				/* we don't care about these things if we're only looking for max amp */
				if ( argc - fileStart > 1 && !applyTrack && !analysisTrack ) {
					/* only check album stuff if more than one file in the list */
					if ( !tagInfo[mainloop].haveAlbumGain ) { albumRecalc |= FULL_RECALC; }
					else if ( tagInfo[mainloop].albumGain != curAlbumGain ) { albumRecalc |= FULL_RECALC; }
				}
				if ( !tagInfo[mainloop].haveTrackGain ) { tagInfo[mainloop].recalc |= FULL_RECALC; }
			}
			if ( argc - fileStart > 1 && !applyTrack && !analysisTrack ) {
				/* only check album stuff if more than one file in the list */
				if ( !tagInfo[mainloop].haveAlbumPeak ) { albumRecalc |= AMP_RECALC; }
				else if ( tagInfo[mainloop].albumPeak != curAlbumPeak ) { albumRecalc |= AMP_RECALC; }
				if ( !tagInfo[mainloop].haveAlbumMinMaxGain ) { albumRecalc |= MIN_MAX_GAIN_RECALC; }
			...
			if ( !tagInfo[mainloop].haveTrackPeak ) { tagInfo[mainloop].recalc |= AMP_RECALC; }
			if ( !tagInfo[mainloop].haveMinMaxGain ) { tagInfo[mainloop].recalc |= MIN_MAX_GAIN_RECALC; }
		}
	}
		...
		if ( checkTagOnly ) {
			...
			if ( !databaseFormat ) {
				...
				if ( curTag->haveTrackGain ) {
					...
					if ( curTag->haveTrackPeak ) { if ( curTag->trackPeak * (Float_t)( pow( 2.0, (double)( intGainChange ) / 4.0 ) ) > 1.0 ) { fprintf( stdout, "WARNING: some clipping may occur with this gain change!\n" ); } }
				}
				...
				if ( curTag->haveAlbumGain ) {
					...
					if ( curTag->haveTrackPeak ) { if ( curTag->trackPeak * (Float_t)( pow( 2.0, (double)( intAlbumGainChange ) / 4.0 ) ) > 1.0 ) { fprintf( stdout, "WARNING: some clipping may occur with this gain change!\n" ); } }
				}
				...
				else { fprintf( stdout, "NA\tNA\t" ); }
				if ( curTag->haveTrackPeak ) { fprintf( stdout, "%f\t", curTag->trackPeak * 32768.0 ); }
				...
				if ( curTag->haveAlbumPeak ) { fprintf( stdout, "%f\t", curTag->albumPeak * 32768.0 ); }
				else { fprintf( stdout, "NA\t" ); }
				if ( curTag->haveAlbumMinMaxGain ) {
			...
			if ( ( tagInfo[mainloop].haveUndo ) && ( tagInfo[mainloop].undoLeft || tagInfo[mainloop].undoRight ) ) {
				...
				if ( databaseFormat ) { fprintf( stdout, "%s\t0\t0\n", argv[mainloop] ); }
				else if ( !QuietMode ) {
					if ( tagInfo[mainloop].haveUndo ) { fprintf( stderr, "No changes to undo in %s\n", argv[mainloop] ); }
					else { fprintf( stderr, "No undo information in %s\n", argv[mainloop] ); }
				}
			}
		}
			...
			if ( whichChannel ) {
				/* do right channel */
				if ( skipTag ) { changeGain( argv[mainloop] AACGAIN_ARG( aacH ), 0, directGainVal ); }
				else { changeGainAndTag( argv[mainloop] AACGAIN_ARG( aacH ), 0, directGainVal, tagInfo + mainloop, fileTags + mainloop ); }
			}
			else {
				/* do left channel */
				if ( skipTag ) { changeGain( argv[mainloop] AACGAIN_ARG( aacH ), directGainVal, 0 ); }
				else { changeGainAndTag( argv[mainloop] AACGAIN_ARG( aacH ), directGainVal, 0, tagInfo + mainloop, fileTags + mainloop ); }
			}
			...
			if ( skipTag ) { changeGain( argv[mainloop] AACGAIN_ARG( aacH ), directGainVal, directGainVal ); }
			else {
				changeGainAndTag( argv[mainloop] AACGAIN_ARG( aacH ),
...
#endif
			{
				...
				if ( useId3 ) { RemoveMP3GainID3Tag( argv[mainloop], saveTime ); }
			}
			if ( ( !QuietMode ) && ( !databaseFormat ) )
		  ...
		  if ((inf == NULL)&&(tagInfo[mainloop].recalc > 0)) {
				...
				if ( tagInfo[mainloop].recalc == 0 ) {
					...
					if ( !( ( tagInfo[mainloop].recalc & FULL_RECALC ) || ( tagInfo[mainloop].recalc & AMP_RECALC ) ) ) {
						/* only min/max rescan */
						maxsample = tagInfo[mainloop].trackPeak * 32768.0;
					}
					else { maxsample = 0; }
				...
				if (!ok) {
					...
					if (tagInfo[mainloop].recalc > 0) {
							...
							while ( ok ) {
								...
								if ( bitridx == 0 ) {
									...
									if ( inbuffer >= bytesinframe ) {
										...
										minGain = &mingain;
										procSamp = 0;
										if ( ( tagInfo[mainloop].recalc & AMP_RECALC ) || ( tagInfo[mainloop].recalc & FULL_RECALC ) ) { decodeSuccess = decodeMP3( &mp, curframe, bytesinframe, &nprocsamp ); }
										else {
									...
									if ( !QuietMode ) { if ( !( ++frame % 200 ) ) { reportPercentAnalyzed( (int)( ( (double)( filepos - ( inbuffer - ( curframe + bytesinframe - buffer ) ) ) * 100.0 ) / gFilesize ), gFilesize ); } }
								}
							}
						}
						...
						else { dBchange = tagInfo[mainloop].trackGain; }
						...
						if ( dBchange == GAIN_NOT_ENOUGH_SAMPLES ) {
							...
							if ( ( !applyTrack ) && ( !applyAlbum ) ) {
								if ( !databaseFormat ) {
									fprintf( stdout, "Recommended \"Track\" dB change: %f\n", dBchange );
									fprintf( stdout, "Recommended \"Track\" mp3 gain change: %d\n", intGainChange );
									if ( maxsample * (Float_t)( pow( 2.0, (double)( intGainChange ) / 4.0 ) ) > 32767.0 ) { fprintf( stdout, "WARNING: some clipping may occur with this gain change!\n" ); }
								...
								if ( intGainChange == 0 ) {
									...
									if ( autoClip ) {
										...
										if ( maxsample * (Float_t)( pow( 2.0, (double)( intGainChange ) / 4.0 ) ) > 32767.0 ) {
											if ( queryUserForClipping( argv[mainloop], intGainChange ) ) { fprintf( stdout, "Applying mp3 gain change of %d to %s...\n", intGainChange, argv[mainloop] ); }
											else { goAhead = 0; }
										}
									}
									if ( goAhead ) {
										fprintf( stdout, "Applying mp3 gain change of %d to %s...\n", intGainChange, argv[mainloop] );
										if ( skipTag ) { changeGain( argv[mainloop] AACGAIN_ARG( aacH ), intGainChange, intGainChange ); }
										else {
											changeGainAndTag( argv[mainloop] AACGAIN_ARG( aacH ),
	...
	if ( ( numFiles > 0 ) && ( !applyTrack ) && ( !analysisTrack ) ) {
		if ( albumRecalc & FULL_RECALC ) {
			...
			dBchange = tagInfo[fileStart].haveAlbumGain ? tagInfo[fileStart].albumGain : tagInfo[fileStart].trackGain;
		}
		...
		if ( dBchange == GAIN_NOT_ENOUGH_SAMPLES ) {
			...
			for ( mainloop = fileStart; mainloop < argc; mainloop++ ) {
				if ( fileok[mainloop] ) {
					if ( tagInfo[mainloop].trackPeak > maxmaxsample ) maxmaxsample = tagInfo[mainloop].trackPeak;
					if ( tagInfo[mainloop].maxGain > maxmaxgain ) maxmaxgain = tagInfo[mainloop].maxGain;
					if ( tagInfo[mainloop].minGain < minmingain ) minmingain = tagInfo[mainloop].minGain;
				}
			}
			...
			if ( !applyAlbum ) {
				if ( !databaseFormat ) {
					...
					for ( mainloop = fileStart; mainloop < argc; mainloop++ ) {
						if ( fileok[mainloop] )
							if ( tagInfo[mainloop].trackPeak * (Float_t)( pow( 2.0, (double)( intGainChange ) / 4.0 ) ) > 1.0 ) { fprintf( stdout, "WARNING: with this global gain change, some clipping may occur in file %s\n", argv[mainloop] ); }
					}
				}
			}
			else {
				...
				for ( mainloop = fileStart; mainloop < argc; mainloop++ ) {
					if ( fileok[mainloop] ) {
						...
						if ( intGainChange == 0 ) {
							...
							if ( goAhead ) {
								fprintf( stdout, "Applying mp3 gain change of %d to %s...\n", intGainChange, argv[mainloop] );
								if ( skipTag ) { changeGain( argv[mainloop] AACGAIN_ARG( aacInfo[mainloop] ), intGainChange, intGainChange ); }
								else { changeGainAndTag( argv[mainloop] AACGAIN_ARG( aacInfo[mainloop] ), intGainChange, intGainChange, tagInfo + mainloop, fileTags + mainloop ); }
							}
							else if ( !skipTag && tagInfo[mainloop].dirty ) {
								fprintf( stdout, "Writing tag information for %s\n", argv[mainloop] );
								WriteMP3GainTag( argv[mainloop] AACGAIN_ARG( aacInfo[mainloop] ), tagInfo + mainloop, fileTags + mainloop, saveTime );
	...
	if ( ( !applyTrack ) &&
		...
		for ( mainloop = fileStart; mainloop < argc; mainloop++ ) { if ( fileok[mainloop] ) { if ( tagInfo[mainloop].dirty ) { WriteMP3GainTag( argv[mainloop] AACGAIN_ARG( aacInfo[mainloop] ), tagInfo + mainloop, fileTags + mainloop, saveTime ); } } }
	}
	...
	for ( mainloop = fileStart; mainloop < argc; mainloop++ ) {
		if ( fileTags[mainloop].apeTag ) {
			if ( fileTags[mainloop].apeTag->otherFields ) { free( fileTags[mainloop].apeTag->otherFields ); }
			free( fileTags[mainloop].apeTag );
		}
		if ( fileTags[mainloop].lyrics3tag ) { free( fileTags[mainloop].lyrics3tag ); }
		if ( fileTags[mainloop].id31tag ) { free( fileTags[mainloop].id31tag ); }
#ifdef AACGAIN
		//close any open aac files
		if ( aacInfo[mainloop] ) aac_close( aacInfo[mainloop] );
#endif
	}
	free( fileTags );
