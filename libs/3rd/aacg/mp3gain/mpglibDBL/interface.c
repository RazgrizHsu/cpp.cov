...
static struct buf *addbuf( PMPSTR mp, unsigned char *ibuf, int size ) {
	...
	if ( !mp->tail ) { mp->tail = nbuf; }
	else { mp->head->next = nbuf; }
...
void copy_mp( PMPSTR mp, int size, unsigned char *ptr ) {
	...
	while ( len < size ) {
		...
		mp->tail->pos += nlen;
		mp->bsize -= nlen;
		if ( mp->tail->pos == mp->tail->size ) { remove_buf( mp ); }
	}
}
...
int decodeMP3( PMPSTR mp, unsigned char *in, int isize, int *done ) {
	...
	if ( in ) { if ( addbuf( mp, in, isize ) == NULL ) { return MP3_ERR; } }
	...
	iret = MP3_NEED_MORE;
	if ( !mp->data_parsed ) {
		if ( mp->dsize > mp->bsize ) { return MP3_NEED_MORE; }
...
#endif
			case 3:
				if ( do_layer3( mp, done ) ) { iret = MP3_ERR; }
				break;
			default:
	...
	if ( bytes > 0 ) {
		...
		size = (int)( wordpointer - ( mp->bsspace[mp->bsnum] + 512 ) );
		if ( size > MAXFRAMESIZE ) { fprintf( stderr, "fatal error.  MAXFRAMESIZE not large enough.\n" ); }
	}
