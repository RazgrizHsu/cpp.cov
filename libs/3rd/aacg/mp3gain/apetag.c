	...
	for ( p = buff; p < end && TagCount--; ) {
...

		{
			if ( !_stricmp( name, "REPLAYGAIN_TRACK_GAIN" ) ) {
				...
				vp = vp + 5; /* skip the comma, too */
				if ( ( *vp == 'w' ) || ( *vp == 'W' ) ) { info->undoWrap = !0; }
				else { info->undoWrap = 0; }
			}
...
int WriteMP3GainAPETag( char *filename, struct MP3GainTagInfo *info, struct FileTagsStruct *fileTags, int saveTimeStamp ) {
	...
	if ( fileTags->apeTag ) {
		...
		if ( fileTags->apeTag->originalTagSize > newTagLength ) {
			...
			if ( !truncate_file( filename, fileTags->tagOffset ) ) { return 0; }
		}
	...
	if ( info->haveUndo ) {
		...
		mp3gainTagData[4] = ',';
		mp3gainTagData += 5;
		if ( info->undoWrap ) { *mp3gainTagData = 'W'; }
		else { *mp3gainTagData = 'N'; }
		mp3gainTagData++;
	}
	if ( info->haveTrackGain ) {
		/* 8 bytes + "REPLAYGAIN_TRACK_GAIN" + '/0' + "+2.456789 dB" = 42 bytes */
	...
	if ( fileTags->lyrics3TagSize > 0 ) { fwrite( fileTags->lyrics3tag, 1, fileTags->lyrics3TagSize, outputFile ); }
