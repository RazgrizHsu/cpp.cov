 ...
 */
static __inline unsigned long id3_get_int32( const unsigned char *p ) { return ( p[0] << 24 ) | ( p[1] << 16 ) | ( p[2] << 8 ) | p[3]; }
...
static struct ID3v2FrameStruct *id3_make_rva2_frame( int is_album_gain, double gain, int have_peak, double peak ) {
	...
	else { return id3_make_frame( "RVA2", "sbbhb", ident, 0, 1, g, 0 ); }
}
...
static int id3_parse_v2_tag( FILE *f, struct ID3v2TagStruct *tag ) {
	...
	if ( ( tag->version >> 8 ) != 4 && ( tag->flags & TAGFL_UNSYNC ) != 0 ) { dlen = id3_get_unsync_data( tagdata, tagdata, dlen ); }
	...
	while ( p < dlen && tagdata[p] != '\0' ) {
		...
		*pframe = frame;
		...
		if ( ( tag->version >> 8 ) == 3 && ( fflags & FRAMEFL_DLEN ) != 0 ) {
			...
			else { id3_put_syncsafe_int( frame->data, k ); }
		}
	}
...
int ReadMP3GainID3Tag( char *filename, struct MP3GainTagInfo *info ) {
	...
	if ( ret == M3G_ERR_READ ) { passError( MP3GAIN_UNSPECIFED_ERROR, 3, "Error reading ", filename, "\n" ); }
